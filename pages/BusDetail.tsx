...
// Add scheduled minutes to a HH:MM string
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const { data: sData } = await supabase
        .from('bus_stops')
        .select('id,stop_name,stop_index,landmark')
        .eq('route_no', routeNo)
      ...
      const { data: bData } = await supabase
        .from('buses')
        .select('id,registration,route_no,status,current_stop_index,seats_occupied,departure_time,arrival_time,driver_name,driver_mobile,delay_mins')
        .eq('route_no', routeNo)
        .order('departure_time', { ascending: true })

      if (rData) setRoute(rData)
      if (sData) setStops(sData)
      if (bData) {
        setBuses(bData)
        // Select bus from URL param or first running bus
        if (busId) {
          const found = bData.find(b => b.id === busId)
          if (found) { setSelBus(found); return }
        }
        const running = bData.find(b => b.status === 'running') || bData[0]
    ...
    <div className="phone-shell">
      ...
      <div style={{ background: 'var(--blue)', padding: '14px 16px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontFamily: 'Rajdhani,sans-serif', fontSize: 26, fontWeight: 700, color: 'white', lineHeight: 1 }}>
              {route.route_no}
            </div>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          ...
          <div style={{ background: '#E8F5E9', color: '#1A7A4A', fontSize: 11, fontWeight: 600, padding: '4px 10px', borderRadius: 20 }}>
            {minsToHHMM(route.duration_mins)}
          </div>
          ...
          <div style={{ display: 'flex', gap: 6, overflowX: 'auto', paddingBottom: 4 }}>
            {buses.map(b => (
              <button key={b.id} onClick={() => setSelBus(b)} style={{
                padding: '5px 12px', borderRadius: 20, border: `1.5px solid ${selBus?.id === b.id ? 'var(--blue)' : '#E2E8F0'}`,
      ...
      <div className="scrollable" style={{ maxHeight: 'calc(100dvh - 220px)' }}>
        ...
        {/* ══════════════════════════════════════
            IRCTC-STYLE VERTICAL ROUTE TRACKER
        ══════════════════════════════════════ */}
        <div style={{ background: 'white', margin: 14, borderRadius: 12, padding: 16, boxShadow: 'var(--shadow)' }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--mute)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 14, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span>🚌 Live Route — Bus Position & Stops</span>
          ...
          {stops.length === 0 ? (
            ...
            <div style={{ position: 'relative', paddingLeft: 44 }}>
              {stops.map((stop, idx) => {
                ...
                // Compute scheduled arrival time for this stop
                const segMins = idx === 0 ? 0 : Math.round((route.duration_mins / (stops.length - 1)) * idx)
                const stopTime = selBus ? addMins(selBus.departure_time, segMins) : '--:--'
                ...
                let nameStyle: React.CSSProperties = {
                  fontSize: 13, fontWeight: 500, color: 'var(--text)', lineHeight: '20px',
                }
                let badge = null
                let connectorStyle: React.CSSProperties = {
                  width: 3, flex: 1, minHeight: 28, background: '#E2E8F0',
                }
                ...
                if (isPassed) {
                  ...
                  connectorStyle = { ...connectorStyle, background: '#1A7A4A' }
                } else if (isCurrent) {
                  circleStyle = {
                    ...circleStyle, width: 24, height: 24,
                    background: 'var(--blue)', borderColor: 'var(--blue)',
                    boxShadow: '0 0 0 4px rgba(27,58,107,0.18)',
                  }
                  ...
                  badge = (
                    <span style={{
                      ...
                      fontSize: 9, fontWeight: 700, padding: '1px 6px', borderRadius: 10, marginLeft: 6, verticalAlign: 'middle',
                    }}>🚌 BUS HERE</span>
                  )
                  connectorStyle = { ...connectorStyle, background: 'linear-gradient(to bottom, var(--blue), #E2E8F0)' }
                } else if (isFirst) {
                  // Boarding stop (first stop = user boards here typically)
                  ...
                  badge = (
                    <span style={{
                      display: 'inline-block', background: '#F5A623', color: 'var(--blue)',
                      fontSize: 9, fontWeight: 700, padding: '1px 6px', borderRadius: 10, marginLeft: 6, verticalAlign: 'middle',
                    }}>📍 START</span>
                  )
                } else if (isDestination) {
                  nameStyle = { ...nameStyle, fontWeight: 600, color: 'var(--text)' }
                  badge = (
                    <span style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <div style={{ position: 'relative', width: 72, height: 72, flexShrink: 0 }}>
              <svg width={72} height={72} style={{ transform: 'rotate(-90deg)' }}>
                ...
                <circle cx={36} cy={36} r={R} fill="none"
                  stroke={occColor(occPct)} strokeWidth={8}
              ...
              <div style={{
                ...
                fontFamily: 'Rajdhani,sans-serif', fontSize: 18, fontWeight: 700, color: 'var(--text)',
              }}>
                {occPct}%
              </div>
            </div>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text)' }}>
                <span style={{ color: '#1A7A4A' }}>{seatsLeft} seats free</span> of {capacity}
              </div>
              <div style={{ fontSize: 11, color: 'var(--mute)', marginTop: 4 }}>
                Derived from ePoS ticket data
              </div>
              <div style={{
                display: 'inline-block', marginTop: 6, padding: '4px 10px',
                borderRadius: 20, fontSize: 11, fontWeight: 600,
                background: occPct >= 85 ? '#FDECEA' : occPct >= 60 ? '#FFF3E0' : '#E8F5E9',
                color: occColor(occPct),
              }}>
                {occMsg(occPct)}
              </div>
            </div>
        ...
        <div style={{ background: 'white', margin: '0 14px 14px', borderRadius: 12, padding: 14, boxShadow: 'var(--shadow)' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0 2px' }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text)' }}>Total Est. Fare</div>
        ...
        {selBus?.driver_name && (
          <div style={{ background: 'white', margin: '0 14px 14px', borderRadius: 12, padding: 14, boxShadow: 'var(--shadow)' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <a href={`tel:${selBus.driver_mobile}`} style={{
                background: 'var(--light)', color: 'var(--blue)', borderRadius: 8,
      ...
      <div style={{
        ...
        background: 'white', borderTop: '1px solid #EEF2F8',
        padding: '12px 14px', display: 'flex', gap: 10, zIndex: 100,
        boxShadow: '0 -4px 20px rgba(13,43,94,0.08)',
      }}>
        {/* Share */}
        <button style={{
          width: 48, background: 'var(--light)', border: 'none',
          borderRadius: 10, color: 'var(--blue)', fontSize: 18, cursor: 'pointer',
        }} onClick={() => {
          if (navigator.share) {
            navigator.share({ title: `Bus ${routeNo}`, text: `Track bus ${routeNo} on APCityPrayaanam`, url: window.location.href })
          }
        }}>↗</button>
        ...
        {booked ? (
          <div style={{
            flex: 1, padding: '13px', background: '#E8F5E9', borderRadius: 10,
            textAlign: 'center', fontFamily: 'Rajdhani,sans-serif', fontSize: 16,
